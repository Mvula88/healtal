-- Fixed migration for 4-tier pricing structure (Lite, Starter, Growth, Premium)
-- This version matches the actual subscription_plans table structure

-- Step 1: Add Lite tier to enum if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- Step 2: Clean up and update subscription_plans table
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'subscription_plans') THEN
    -- Remove existing entries to avoid conflicts
    DELETE FROM subscription_plans WHERE tier IN ('lite', 'starter', 'growth', 'premium');
    ...
    ELSE
      -- Table doesn't have price_yearly column
...
-- Step 5: Update existing users to new tier structure
UPDATE public.users 
...
WHERE subscription_tier IS NULL 
   OR subscription_tier NOT IN ('lite', 'starter', 'growth', 'premium');
...
-- Step 7: Create usage limit checking function
...
-- Step 9: Function to increment usage
